...
void Island::Solve(Profile* profile, const TimeStep& step, const Vec2& gravity, bool allowSleep)
{
	...
	for (int32 i = 0; i < m_bodyCount; ++i)
	{
		...
		{
			float32 ratio = maxTranslation / translation.Length();
			v *= ratio;
		}
		...
		{
			float32 ratio = maxRotation / Abs(rotation);
			w *= ratio;
		}
	...
	if (allowSleep)
	{
		float32 minSleepTime = maxFloat;

		const float32 linTolSqr = linearSleepTolerance * linearSleepTolerance;
		const float32 angTolSqr = angularSleepTolerance * angularSleepTolerance;
