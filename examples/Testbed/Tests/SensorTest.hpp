...
public:
	...
	{
		{
			...
			{
				b2::FixtureDef sd;
				sd.SetAsBox(10.0f, 2.0f, b2::Vec2(0.0f, 20.0f), 0.0f);
				sd.isSensor = true;
	...
	void Step(Settings* settings)
	{
		...
		for (b2::int32 i = 0; i < e_count; ++i)
		{
			...
			b2::Body* body = m_bodies[i];
			b2::Body* ground = m_sensor->GetBody();

			b2::CircleShape* circle = (b2::CircleShape*)m_sensor->GetShape();
			b2::Vec2 center = ground->GetWorldPoint(circle->m_p);

			b2::Vec2 position = body->GetPosition();

			b2::Vec2 d = center - position;
