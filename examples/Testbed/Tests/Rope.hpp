...
public:
	...
	void Keyboard(unsigned char key)
	{
		...
		{
		case 'q':
			m_angle = b2::Max(-b2::pi, m_angle - 0.05f * b2::pi);
			m_rope.SetAngle(m_angle);
			break;

		case 'e':
			m_angle = b2::Min(b2::pi, m_angle + 0.05f * b2::pi);
			m_rope.SetAngle(m_angle);
			break;
		}
	}
	...
	{
		b2::float32 dt = settings->hz > 0.0f ? 1.0f / settings->hz : 0.0f;
		...
		m_debugDraw.DrawString(5, m_textLine, "Target angle = %g degrees", m_angle * 180.0f / b2::pi);
		m_textLine += DRAW_STRING_NEW_LINE;
	}
