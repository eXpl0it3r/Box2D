...
public:
	ApplyForce()
	{
		...
		const b2::float32 k_restitution = 0.4f;
		...
		b2::Body* ground;
		{
			...
			// Left vertical
			shape.Set(b2::Vec2(-20.0f, -20.0f), b2::Vec2(-20.0f, 20.0f));
...

		{
			...
			for (int i = 0; i < 10; ++i)
			{
				...
				b2::float32 gravity = 10.0f;
				b2::float32 I = body->GetInertia();
				b2::float32 mass = body->GetMass();

				// For a circle: I = 0.5 * m * r * r ==> r = sqrt(2 * I / m)
				b2::float32 radius = std::sqrt(2.0f * I / mass);

				b2::FrictionJointDef jd;
	...
	void Keyboard(int key)
	{
		...
		case GLFW_KEY_W:
			{
				b2::Vec2 f = m_body->GetWorldVector(b2::Vec2(0.0f, -200.0f));
				b2::Vec2 p = m_body->GetWorldPoint(b2::Vec2(0.0f, 2.0f));
