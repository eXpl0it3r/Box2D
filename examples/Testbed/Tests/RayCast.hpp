...
class RayCastClosestCallback : public b2::RayCastCallback
{
public:
	...
	b2::float32 ReportFixture(b2::Fixture* fixture, const b2::Vec2& point, const b2::Vec2& normal, b2::float32 fraction)
	{
		b2::Body* body = fixture->GetBody();
		void* userData = body->GetUserData();
		if (userData)
		{
			b2::int32 index = *(b2::int32*)userData;
			if (index == 0)
			{
...
class RayCastAnyCallback : public b2::RayCastCallback
{
public:
	RayCastAnyCallback()
	{
...
// the closest fixture.
class RayCastMultipleCallback : public b2::RayCastCallback
{
public:
	enum
	{
	...
	b2::Vec2 m_points[e_maxCount];
	b2::Vec2 m_normals[e_maxCount];
	b2::int32 m_count;
};
...
public:
	...
	RayCast()
	{
		...
		{
			m_edge.Set(b2::Vec2(-1.0f, 0.0f), b2::Vec2(1.0f, 0.0f));
		}
	...
	void Create(b2::int32 index)
	{
		...
		b2::float32 y = RandomFloat(0.0f, 20.0f);
		bd.position.Set(x, y);
	...
	void Step(Settings* settings)
	{
		...
		b2::Vec2 d(L * std::cos(m_angle), L * std::sin(m_angle));
		...
		else if (m_mode == e_multiple)
		{
			...
			for (b2::int32 i = 0; i < callback.m_count; ++i)
			{
				b2::Vec2 p = callback.m_points[i];
				b2::Vec2 n = callback.m_normals[i];
				g_debugDraw.DrawPoint(p, 5.0f, b2::Color(0.4f, 0.9f, 0.4f));
				g_debugDraw.DrawSegment(point1, p, b2::Color(0.8f, 0.8f, 0.8f));
				b2::Vec2 head = p + 0.5f * n;
				g_debugDraw.DrawSegment(p, head, b2::Color(0.9f, 0.9f, 0.4f));
			}
		}
		...
		// This case was failing.
		{
			...
			b2::Color color(1.0f, 1.0f, 1.0f);
			b2::Vec2 vs[4];
			for (b2::int32 i = 0; i < 4; ++i)
			{
				vs[i] = b2::Mul(xf, shape.m_vertices[i]);
			}
	...
	b2::int32 m_userData[e_maxBodies];
