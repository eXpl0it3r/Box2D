...
#ifndef DUMP_SHELL_HPP
#define DUMP_SHELL_HPP

// This test holds worlds dumped using b2::World::Dump.
...
public:
	...
	DumpShell()
	{
		...
		//  a b2::World for use as the 'm_world' variable in the code below.

		b2::Vec2 g(0.000000000000000e+00f, -1.000000000000000e+01f);
		m_world->SetGravity(g);
		b2::Body** bodies = (b2::Body**)b2::Alloc(3 * sizeof(b2::Body*));
		b2::Joint** joints = (b2::Joint**)b2::Alloc(0 * sizeof(b2::Joint*));
		{
			b2::BodyDef bd;
			bd.type = b2::BodyType(0);
...

			{
				...
				fd.isSensor = bool(0);
				fd.filter.categoryBits = b2::uint16(1);
				fd.filter.maskBits = b2::uint16(65535);
				fd.filter.groupIndex = b2::int16(0);
		...
		}
		{
...

			{
				...
				fd.filter.categoryBits = b2::uint16(1);
				fd.filter.maskBits = b2::uint16(65535);
				fd.filter.groupIndex = b2::int16(-3);
				b2::PolygonShape shape;
				b2::Vec2 vs[8];
